...
#define MQTT_TOPIC_CMD               "car/instructions"         // instrucciones (de tu código original)
#define MQTT_TOPIC_DISTANCE          "car/telemetry/distance"   // NUEVO: telemetría (distinto a instrucciones)
 
// HTTP
#define HTTP_PORT                    80
 
// Motores (tu cableado original)
#define PIN_MOTOR1A                  19  // IN1 motor 1
#define PIN_MOTOR1B                  21  // IN2 motor 1
#define PIN_MOTOR2A                  26  // IN1 motor 2
#define PIN_MOTOR2B                  25  // IN2 motor 2
...
float readDistanceCm();         // requerida: sin argumentos, retorna float
...
void setup() {
  ...
  server.on("/control", [](){
    ...
    // Publica en topic de instrucciones (igual que tu versión anterior)
...
void ensureMQTT() {
  ...
  while (!mqttClient.connected()) {
    ...
    if (mqttClient.connect(clientId.c_str())) {
      ...
      // Si quisieras suscribirte a algo adicional, hazlo aquí.
    } else {
